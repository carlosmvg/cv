...
sap.ui.define([
	...
	"./GanttUtils",
	"sap/ui/core/Core"
], function (BaseText, AggregationUtils, GanttUtils, Core) {
	"use strict";
	...
	var RenderUtils = {
		...
		renderElementTitle : function(oRm, oElement) {
			...
			if (sTitle) {
				...
				//Set x cocordinate for the text to be displayed in RTL mode.
				//Adding the width of the shape would result in the Text to be added to the end of the Shape.
				if (Core.getConfiguration().getRTL() == true) {
					mTextSettings.x = oElement.getX() + iShapeWidth - iCornerPaddingPixel;
				} else {
