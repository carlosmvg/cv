	...
	var FioriHelperHandler = function() {
		...
		this.openSaveTileDialog = function(oControlProperties) {
            ...
            var loSaveTileButton = new sap.ushell.ui.footerbar.AddBookmarkButton({
            	...
            	afterPressHandler: function() {
            		...
            		if (that.sOntilesettingssubmittedcommand && that.sOntilesettingssubmittedcommand.length > 0) {
            			var loFuncAction = new Function(that.sOntilesettingssubmittedcommand);
            			loFuncAction();
            		}
            	}
            });
		...
		this.saveTile = function(oControlProperties) {
			...
			if (loBookmarkService) {
				var lHash = typeof hasher !== "undefined" ? hasher.getHashAsArray() : [""];
				if (oControlProperties.usefrontendtilesettings) {
					if (lUrlParameters) {
						...
						this.oBookmarkTileData.url = this.mixInURLParameters(lHash, lUrlParameters);
					}
					oBookmarkService.addBookmark(this.oBookmarkTileData, this.oBookmarkTileGroup);
				} else {
					var lUrl = this.mixInURLParameters(lHash, lUrlParameters);
