sap.ui.define(["sap/ovp/cards/generic/Card.controller", "sap/ovp/cards/charts/VizAnnotationManager",
    "sap/viz/ui5/data/FlattenedDataset", "sap/ovp/cards/OVPCardAsAPIUtils", "sap/ovp/app/resources", "sap/base/util/each", "sap/ovp/cards/ovpLogger"],
    function (CardController, VizAnnotationManager, FlattenedDataset, OVPCardAsAPIUtils, OvpResources, each, ovpLogger) {
        "use strict";
        var Log = new ovpLogger("OVP.analytical.analyticalChart");
...
//            var handler = vizFrame.getParent();
            ...
            resizeCard: function (newCardLayout) {
                ...
                this.getView().getDomRef().querySelectorAll(".sapOvpWrapper").forEach(function(elem) {
                    elem.style.height = (newCardLayout.rowSpan * oCardLayout.iRowHeightPx + 2) - (iHeaderHeight + 2 * oCardLayout.iCardBorderPx) + "px";
                });
                var vizCard = this.getView().byId("analyticalChart");
                var bubbleText = this.getView().byId("bubbleText");
                var chartTitle = this.getView().byId("ovpCT");
