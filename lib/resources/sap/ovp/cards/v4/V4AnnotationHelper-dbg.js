...
sap.ui.define(["sap/ui/core/format/NumberFormat", "sap/ui/core/format/DateFormat",
    ...
    function (NumberFormat, DateFormat, CommonUtils, OdataAnnotationHelper,
        ...
        var Log = new ovpLogger("OVP.cards.V4AnnotationHelper");
        ...
        function formatDataField(iContext, aCollection, index) {
            ...
            if (item) {
                return formatField(iContext.getInterface(0), item);
            }
            return "";
        }
        ...
        function _formatField(iContext, oItem, oEntityType, oMetaModel, bDontIncludeUOM, bIncludeOnlyUOM, bUseSimplePath) {
            ...
            var oEntityTypeProperty = sProperty,
                result = "",
                functionName,
                oStaticValues;
            ...
            if (!bIncludeOnlyUOM) {
                ...
                if (oEntityTypeProperty["type"] === 'Edm.DateTime' || oEntityTypeProperty["type"] === 'Edm.DateTimeOffset') {
                    ...
                    if (bUseSimplePath) {
                        ...
                        var sFormattedResult = OdataAnnotationHelper.format(oItem.Value, {context: iContext});
        ...
        function formatItems(iContext, oEntitySet) {
            ...
            var oSelectionVariant = modelData["$Annotations"][oEntityType][selectionAnnotationPath];
            // var presentationAnnotationPath = oModel.getProperty('/presentationAnnotationPath');
            var oPresentationVariant = "@" + modelData["$Annotations"][oEntityType][selectionAnnotationPath];
        ...
        function getSelectFields(iContext, oMetaModel, oEntityType, aAnnotationsPath) {
            ...
            for (var i = 0; i < aAnnotationsPath.length; i++) {
                ...
                sAnnotationPath = oEntityType.split(".")[1] + "/@" + aAnnotationsPath[i];
                    ...
                    if (aItemValue && aItemValue.length > 0) {
                        ...
                        for (var k = 0; k < aItemValue.length; k++) {
                            ...
                            if (!aSelectFields[sItemValue]) {
                                ...
                                var sUnitPropName = CommonUtils.getUnitColumn(sItemValue, {property: oBindingContext.getObject("/" + oEntityType.split(".")[1] + "/@")});
        ...
        function getAttributeValueForEntityProperty(oMetamodel, oEntityType, sPropertyName, sAttributeName) {
            var oProp = oMetamodel.getData(sPropertyName).$Annotations[oEntityType + "/" + sPropertyName];
            if (!oProp) {
                Log.error("No Property Found for with Name '" + sPropertyName + " For Entity-Type '" + oEntityType.name + "'");
                return;
            }
            return oProp["@com.sap.vocabularies.Common.v1.Label"];
        }
