...
sap.ui.define([
	...
	AxisAngleRotationToolGizmo.prototype._createGizmoObject = function() {
		...
		for (i = 0; i < 3; i++) {
			...
			arcMesh.drawMode = THREE.TriangleFanDrawMode;
	...
	function updateArcMesh(arcMesh, axisIndex, angle1, angle2) {
		...
		var n = Math.min(Math.max(Math.ceil(Math.abs(deltaAngle) * 64 / Math.PI), 1), 10000);
		...
		arcMesh.geometry = new THREE.BufferGeometry().setAttribute("position", new THREE.Float32BufferAttribute(vertices, 3));
