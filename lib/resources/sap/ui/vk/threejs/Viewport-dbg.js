...
sap.ui.define([
	...
	NodeContentType
		...
		*/
		...
		Viewport.prototype._isPanoramicActivated = function() {
			var nativeCamera = this._getNativeCamera();
			return nativeCamera ? nativeCamera.userData.rotate : false;
		};
		...
		Viewport.prototype.tap = function(x, y, isDoubleClick) {
			...
			} else if (!this.getFreezeCamera() && !this._isRedlineActivated() && !this._isPanoramicActivated()) {
	...
	// remove selection for snapshot
		...
		*/
		...
		Viewport.prototype.rotate = function(dx, dy, isTurntable) {
			this._viewportGestureHandler._cameraController.rotate(dx, dy, isTurntable);
		};
		...
		Viewport.prototype.getSymbolNodes = function(nodeId) {
			var view = this.getCurrentView() || this.getScene().getViews()[0];
			var symbolNodes = view ? view.getNodeInfos().reduce(function(nodeInfos, currNode) {
				if (currNode.target._vkGetNodeContentType() === NodeContentType.Symbol) {
