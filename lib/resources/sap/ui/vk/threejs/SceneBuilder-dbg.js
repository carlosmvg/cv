...
sap.ui.define([
	...
	*/
	...
	SceneBuilder.prototype.createAnnotation = function(annotation, sceneId) {
		...
		var originalTransform = node.userData.originalTransform;
		if (originalTransform) {// if node has ORTHO2D transformType, then we need to restore its original transformation because it can be changed
			node.position.copy(originalTransform.p);
			node.scale.copy(originalTransform.s);
		}
	...
	SceneBuilder.prototype.updateTextureMap = function(materialId, type) {
		...
		var repeatS = (info.uvHorizontalScale !== undefined && info.uvHorizontalScale !== 0) ? info.uvHorizontalScale : 1;
		var repeatT = (info.uvVerticalScale !== undefined && info.uvVerticalScale !== 0) ? info.uvVerticalScale : 1;
		...
		function updateMaterialTexture(m) {
			switch (type) {
				case SceneBuilder.TextureType.Diffuse:
		...
		if (materialClones) {
			materialClones.forEach(function(materialClone) {
				...
				updateMaterialTexture(materialClone);
			});
		}
