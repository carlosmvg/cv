...
sap.ui.define([
	 ...
	 * @protected
	 */
	ActionUtil.prototype.call = function (sFunctionImportPath, sFunctionImportLabel, bIsDraftEnabled, oSkipProperties) {
		var that = this;
			...
			if (that._isActionCritical()) {
				...
				MessageBox.confirm(sMsgBoxText, {
					...
					onClose: function (oAction) {
						if (oAction === "OK") {
							mActionParams = that._prepareParameters(that.getContexts(), bIsDraftEnabled);
							mActionParams = mActionParams || [{}];
							that._initiateCall(mActionParams, bIsDraftEnabled);
				...
				mActionParams = that._prepareParameters(aContexts, bIsDraftEnabled);
				mActionParams = mActionParams || [{}];
				that._initiateCall(mActionParams, bIsDraftEnabled);
			}
		});
	 ...
	 * @private
	 */
	ActionUtil.prototype._prepareParameters = function (aContexts, bIsDraftEnabled) {
		/**
	 ...
	 *
	 */
	ActionUtil.prototype._initiateCall = function (mActionParams, bIsDraftEnabled) {
		...
		if (mActionParam != undefined && mActionParam.additionalParameters && mActionParam.additionalParameters.length == 0) {
			this._call(mActionParam.parameterData, mActionParam.isDraftEnabled);
			...
			var oEntityContext = this.getContexts();
			var fnChangeSet = this.getApplicationController()._getChangeSetFunc(oEntityContext, this.getOperationGrouping());

			var oFuncHandles = oEntityContext.map(function(oContext, index) {
							...
							if (mParameterForm.hasNoClientErrors()) {
								if (mParameterForm.getEmptyMandatoryFields().length == 0) {
									...
									that._oActionPromiseCallback.resolve({
										executionPromise: Promise.all(oFuncHandles.map(function (oFuncHandle) {
											return oFuncHandle.result;
										}))
										.then(function (aResults) {
											that._bExecutedSuccessfully = true;
											if (aResults.length === 1 && aResults[0].context) {
												aResults[0].response = Object.assign(aResults[0].response, {context: aResults[0].context});
											}
											return aResults;
										}, function (oError) {
									...
									oEntityContext.forEach(function (ctx, i) {
			...
			this._call(null, mActionParam != undefined ? mActionParam.isDraftEnabled : bIsDraftEnabled);
		}
	};
