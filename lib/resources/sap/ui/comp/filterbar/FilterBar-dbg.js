		...
		return new Promise(function(fResolve) {
			...
			this._getEntitiesLazy().then(function() {
				...
				if (!this._oAdaptFiltersDialog) {
					...
					this._oAdaptFiltersDialogModel = new JSONModel(oP13nData);
					...
					var oFilterPanel = new GroupPanelBase({
						change: fChange,
						//onReset: this._dialogRestore.bind(this),
						expandFirstGroup: true
					});

					if (this.getShowRestoreButton() && this._oVariantManagement && this._oVariantManagement.getVisible()) {
						oFilterPanel.setOnReset(this._dialogRestore.bind(this));
					}
...
//						this._adaptMandatoryForFilter(oFilterItem);
					...
					var fOk = function(oEvent) {
						this._dialogSearch(this._oAdaptFiltersDialog);
					}.bind(this);

					// create adapt dialog
					P13nBuilder.createP13nDialog(oFilterPanel, {
						id: this.getId() + "-adapt-filters-dialog",
						//title: this._oRb.getText("FILTER_BAR_ADAPT_FILTERS_DIALOG"),
