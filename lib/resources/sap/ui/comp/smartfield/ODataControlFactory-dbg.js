...
sap.ui.define([
	...
	ODataControlFactory.prototype._getTextArrangementType = function () {
		var mTextArrangementBindingPaths = TextArrangementDelegate.getPaths("ValueList", this._oMetaData);
		if (
			mTextArrangementBindingPaths.keyField &&
			mTextArrangementBindingPaths.descriptionField
		) {
			return this._oTypes.getType(this._oMetaData.property, null, null, {
				composite: true,
	...
	ODataControlFactory.prototype._checkTextInDisplayModeValueList = function () {
		// Smoke check if we are in a value list with non-fixed list scenario
		return !!(
			!this._oSelector.checkComboBox().combobox && // We are in a non-fixed list scenario
			this._oParent.getMode() === "display" &&
			this._oParent.getFetchValueListReadOnly() && // Fetch property is not turned off
			this._oHelper.getValueListAnnotationPath(this._oMetaData) && // We have a value list annotation path
			!this._oHelper.oAnnotation.getText(this.getEdmProperty()) // There should be no text annotation assigned to the main property
		);
	};
		...
		.then(function onValueListAnnotationLoaded(oValueListAnnotations) {
			...
			if (oValueListAnnotations && this.oTextArrangementDelegate) {
				...
				this._handleModelContextChangeEvent = function () {
					// Only when the SmartField has a binding context
					if (this._oParent && this._oParent.getBindingContext()) {
						this.oTextArrangementDelegate.fetchIDAndDescriptionCollectionIfRequired();
					}
				}.bind(this);
