    ...
    function start (oUi5BootPromise) {
        ...
        oStartupPromise = oStartupHandler.requestStartupConfig().then(function (oStartupResult) {
            ...
            if (jQuery.sap.getObject("sap-ushell-config.ushell.spaces.enabled")) {
                ...
                jQuery.sap.setObject("sap-ushell-config.services.VisualizationDataProvider.adapter.config", oLaunchPageAdapterConfig);
                //NavigationDataProvider and ClientSideTargetResolutionAdapter should have the same config
                jQuery.sap.setObject("sap-ushell-config.services.NavigationDataProvider.adapter.config", oClientSideTargetResolutionAdapterConfig);
            }
            ...
            if (isHomepageHash(sHash)) {
                if (jQuery.sap.getObject("sap-ushell-config.ushell.spaces.enabled")) {
                    ...
                    The same "/UI2/FakePageId" is used in LaunchPageAdapter
                    */
                    oCatalogHandler.requestAllCatalogs(oStartupResult, bNoOData, "/UI2/FakePageId");
                } else {
