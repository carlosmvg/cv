...
sap.ui.define(
	...
	function(
		 ...
		 * @param {string} [sActionName] the injected action Name
		 ...
		 * @sap-restricted
		 */
		function executeDraftActivationAction(sActionName, sGroupId) {
			if (!this.getProperty("IsActiveEntity")) {
				var oOperation = createOperation(this, arguments[0], { $$inheritExpandSelect: true });
				return oOperation.execute(sGroupId).then(function(oActiveDocumentContext) {
		 ...
		 * @sap-restricted
		 */
		function executeDraftPreparationAction(sActionName, sideEffectsQualifier, sGroupId) {
			if (!this.getProperty("IsActiveEntity")) {
		 ...
		 *
		 * @function
		 * @name sap.fe.core.model.DraftModel.upgradedContext#executeDraftDiscardActiom
		 * @returns {Promise.<sap.ui.model.odata.v4.Context>} Resolve function returns the context of the operation
		 * @private
		 * @sap-restricted
		 */
		function executeDraftDiscardAction() {
			if (!this.getProperty("IsActiveEntity")) {
				var oOperation = createOperation(this, arguments[0]);
		...
		var oOperationTemplates = {
			...
			"DiscardAction": executeDraftDiscardAction,
