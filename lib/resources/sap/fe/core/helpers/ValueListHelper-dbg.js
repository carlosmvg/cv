...
sap.ui.define(
	...
	function(
		...
		var ValueListHelper = {
			...
			getTableItemsParameters: function(oValueList, sRequestGroupId) {
				var sSortFieldName,
					sParameters = "",
				...
				var bSuspended = oValueList.Parameters.some(function(oParameter) {
					return oParameter.$Type === "com.sap.vocabularies.Common.v1.ValueListParameterIn";
				});
			...
			createValueHelpDialog: function(propertyPath, oFVH, oTable, oFilterBar, oValueListInfo, bSuggestion) {
				...
				return Promise.all([oTable, oFilterBar]).then(function(aControls) {
					...
					if (oFilterBar && oFilterBar !== oFVH.getFilterBar() && aFilterItems.length) {
			...
			showValueListInfo: function(propertyPath, oFVH, bSuggestion, sConditionModel) {
				...
				if (!oFilterBar && oFVH.getDependents().length > 0) {
					...
					if (oPotentialFilterBar.isA("sap.ui.mdc.FilterBar")) {
						...
						.then(function(oValueListInfo) {
							if (
								...
								oPropertyAnnotations =
									mValueListInfo.$model.getMetaModel().getObject(oValueListInfo.fieldPropertyPath + "@") || {};
								oCollectionAnnotations =
									mValueListInfo.$model.getMetaModel().getObject("/" + mValueListInfo.CollectionPath + "/$Type@") || {};
