sap.ui.define(
	...
	function(FilterBarAPI, Utils, FEBuilder, OpaBuilder, EditState) {
		...
		/**
		 * Checks a filter field is not rendered.
		 * @param {object | sap.fe.test.api.FilterFieldIdentifier} vFieldIdentifier the field identifier
		 * <code><pre>
		 * 	{
		 * 		focused: true|false
		 ...
		 * @sap-restricted
		 */
		FilterBarAssertions.prototype.iCheckNoFilterField = function(vFieldIdentifier) {
			...
			return this.prepareResult(
				oFilterBarBuilder
					.has(function(oFilterBar) {
						return !oFilterBar.getFilterItems().some(function(oFilterItem) {
							return oFilterItem.getFieldPath() === aArguments[0];
						});
					})
					.description(
						Utils.formatMessage(
							"Checking the filter field '{1}' of filter bar '{0}' is not rendered in the filterBar",
							this.getIdentifier(),
							aArguments[0]
						)
					)
