sap.ui.define(
	...
	function(TableAPI, Utils, OpaBuilder, FEBuilder, TableBuilder, SortOrder) {
		"use strict";
		...
		TableAssertions.prototype.iCheckPaste = function(mState) {
			...
			return this.prepareResult(
				oTableBuilder
					.hasAggregation("actions", [
						FEBuilder.Matchers.id(new RegExp(Utils.formatMessage("{0}$", sPasteId))),
						FEBuilder.Matchers.states(mState)
					])
					.description(Utils.formatMessage("Checking action '{0}' with state='{1}'", sPasteId, mState))
					.execute()
			);
		};
