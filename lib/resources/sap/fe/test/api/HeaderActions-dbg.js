	...
	function(HeaderAPI, Utils, OpaBuilder, FEBuilder, ShareUtilsHelper) {
		...
		HeaderActions.prototype.iExecuteRelatedApps = function() {
			...
			return this.prepareResult(
				oOverflowToolbarBuilder
					.doOnContent(
						OpaBuilder.Matchers.resourceBundle("text", "sap.fe.templates", "T_OP_REALATED_APPS"),
		...
		/**
		 * Execute a Related Apps action in the ObjectPage Header Toolbar. The action is identified by a string representing
		 * the label of the action.
		 *
		 * @param {string} [sItem] the label of the link
		 * @returns {object} an object extending a jQuery promise.
		 *
		 * @private
		 * @experimental
