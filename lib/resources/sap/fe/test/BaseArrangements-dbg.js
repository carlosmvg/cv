sap.ui.define(
	...
	function(Opa5, OpaBuilder, jQuery, UriParameters, Utils, Stubs) {
		...
		return Opa5.extend("sap.fe.integrations.common.BaseArrangements", {
			iStartMyApp: function(appName, sAppParams, mSandboxParams, bNotInternal) {
				// using NodeJS Mockserver on 4004 as backend
				var oUriParams = new UriParameters(window.location.href),
					sPathToDemokit = bNotInternal ? "/" : "/internal/",
					sBackendUrl = oUriParams.get("useBackendUrl") ? "&useBackendUrl=" + oUriParams.get("useBackendUrl") : "",
				...
				if (!appName) {
					this.iStartMyAppInAFrame(
						"test-resources/sap/fe/templates/internal/demokit/appStarter.html?sap-ui-log-level=ERROR&sap-ui-xx-viewCache=true" +
							sBackendUrl +
							"&app=" +
							"SalesOrder-manage" +
							(sAppParams || "")
					...
					this.iStartMyAppInAFrame(
						"test-resources/sap/fe/templates" +
							sPathToDemokit +
							"demokit/flpSandbox.html?sap-ui-log-level=ERROR&sap-ui-xx-viewCache=true" +
							sBackendUrl +
							sSandBoxParams +
							"#" +
							appName +
