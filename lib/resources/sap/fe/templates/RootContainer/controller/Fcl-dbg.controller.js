sap.ui.define(
	...
	function(Log, JSONModel, BaseController, FlexibleColumnLayoutSemanticHelper, Component, Routing) {
		"use strict";
		...
		return BaseController.extend("sap.fe.templates.RootContainer.controller.Fcl", {
			onInit: function(oEvent) {
				...
				this._aFCLHelperModels = [];
			},

			attachShellTitleHandler: function() {
				BaseController.prototype.attachShellTitleHandler.apply(this, arguments);
				var oAppComponent = Component.getOwnerComponentFor(this.getView());
			...
			_createFCLHelperModel: function() {
				...
				this._aFCLHelperModels.push(oModel);
			...
			onExit: function() {
				...
				this._aFCLHelperModels.forEach(function(oModel) {
					oModel.destroy();
				});
			},
			...
			onRouteMatched: function(oEvent) {
				...
				this._updateUIStateForEachView();
			...
			_updateUIStateForEachView: function() {
				var aVisibleViews = this._getAllVisibleViews();

				if (!this.getView().getModel("fclnavigated")) {
					this.getView().setModel(this._createFCLHelperModel(), "fclnavigated");
				}

				for (var index = 0; index < aVisibleViews.length; index++) {
					var oView = aVisibleViews[index];
					if (oView) {
						this._updateUIStateForView(oView);

						if (oView.getBindingContext()) {
							var sDeepestPath = this.getView()
									.getModel("fclnavigated")
									.getProperty("/deepestPath"),
								sViewPath = oView.getBindingContext().getPath();

							if (!sDeepestPath || sDeepestPath.indexOf(sViewPath) !== 0) {
								...
								this.getView()
									.getModel("fclnavigated")
									.setProperty("/deepestPath", sViewPath);
							}
						}
					}
				}
			...
			_updateUIStateForView: function(oView) {
				var oUIState = this.getHelper().getCurrentUIState(),
					...
					FCLLevel = oView.getController().fcl.getFCLLevel(),
					sLayout = this.getFclControl().getLayout(),
					viewColumn;

				if (!oView.getModel("fclhelper")) {
					oView.setModel(this._createFCLHelperModel(), "fclhelper");
				}
				if (FCLLevel >= this._oFCLConfig.maxColumnsCount) {
					//if (FCLLevel >= oUIState.maxColumnsCount) {
			...
			updateFCLLevels: function(aTargetNames, aContainers) {
				var oView;
				if (aTargetNames.length == 1 && this.getTargetAggregation()[aTargetNames[0]].aggregation !== "beginColumnPages") {
					// Only 1 view in the last column : FCLLevel forced to 3 (fullscreen for 2 or 3 columns)
			 ...
			 *
			 * @name sap.fe.core.controllerextensions.FlexibleColumnLayout#_getAllVisibleViews
			 * @memberof sap.fe.core.controllerextensions.FlexibleColumnLayout
			 * @returns {Array} return views
			 */

			_getAllVisibleViews: function() {
				var aViews = [];
				...
				switch (this.getFclControl().getLayout()) {
					case sap.f.LayoutType.EndColumnFullScreen:
						if (this.getFclControl().getCurrentEndColumnPage()) {
							aViews.push(this._getViewFromContainer(this.getFclControl().getCurrentEndColumnPage()));
						}
						break;

					case sap.f.LayoutType.MidColumnFullScreen:
						if (this.getFclControl().getCurrentMidColumnPage()) {
							aViews.push(this._getViewFromContainer(this.getFclControl().getCurrentMidColumnPage()));
						}
						break;

					case sap.f.LayoutType.OneColumn:
						if (this.getFclControl().getCurrentBeginColumnPage()) {
							aViews.push(this._getViewFromContainer(this.getFclControl().getCurrentBeginColumnPage()));
						}
						break;
