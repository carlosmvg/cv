...
sap.ui.define(
	...
	function(NavError, SelectionVariant, NavLibrary, BaseObject, UIComponent, HashChanger, extend, isEmptyObject, Log, assert, merge, URI) {
		"use strict";
		...
		var NavigationHandler = BaseObject.extend("sap.fe.navigation.NavigationHandler" /** @lends sap.fe.navigation.NavigationHandler */, {
			 ...
			 * @param {boolean} [bIsStickyEditMode] Parameter used to defined if the app is on sticky mode.
			 * @private
			 * @sap-restricted <br>
			 ...
			 * @param {boolean} bMeasure should measures be removede
			 * @returns {object} selection variant
			 **/
			_removeSensitiveDataForODataV4: function(oData, bMeasure) {
				...
				if (
					...
					var oEntitySetProperties = oMetaModel.getObject("/" + sEntitySet + "/");
					for (var k = 0; k < aPropertyNames.length; k++) {
						var sProperty = aPropertyNames[k];
						// properties of the entity set
						if (
							oEntitySetProperties[sProperty] &&
							oEntitySetProperties[sProperty].$kind &&
							oEntitySetProperties[sProperty].$kind === "Property" &&
							fnIsSensitiveData(sProperty, sEntitySet)
						) {
							oSV.removeSelectOption(sProperty);
						} else if (
							oEntitySetProperties[sProperty] &&
							oEntitySetProperties[sProperty].$kind &&
							oEntitySetProperties[sProperty].$kind === "NavigationProperty"
						) {
							...
							var esName = "/" + oMetaModel.getObject("/" + sEntitySet + "/$NavigationPropertyBinding/" + sProperty),
								oNavigationEntityData = JSON.parse(oSV.getSelectOption(sProperty)[0].Low),
							...
							for (var p = 0; p < aProps.length; p++) {
								var sProp = aProps[p];
								if (fnIsSensitiveData(sProp, esName)) {
									...
									oSV.removeSelectOption(sProperty);
									oSV.addSelectOption(sProperty, "I", "EQ", JSON.stringify(oNavigationEntityData));
								}
							}
						}
					}
