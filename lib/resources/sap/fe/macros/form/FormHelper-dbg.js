...
sap.ui.define(
	...
	function(ODataModelAnnotationHelper, CommonHelper) {
		...
		var FormHelper = {
			 ...
			 * Example:
			 * 1. FormHelper.resolveAnnotationPathForForm(oContext) : '/Products/$NavigationPropertyBinding/to_supplier/@com.sap.vocabularies.UI.v1.FieldGroup'
			 *    return : '/Supplier'
			 */
			getTargetEntitySet: function(oContext) {
				var sPath = FormHelper.resolveAnnotationPathForForm(oContext),
					sEntitySetPath = ODataModelAnnotationHelper.getNavigationPath(sPath);
			...
			/**Checks whether the Facet has content in addtion to Actions and IBN and
			 * will be displayed with content.
			 * @param {Array} aDataFields - The Fields defined for this Data Field Collection
			 * @returns {boolen} true if DataField contains content other than Actions and IBN
			 */
			dataFieldCollectionContainsMoreThanActionOrIBN: function(aDataFields) {
				return aDataFields.some(function(oDataField) {
					return (
			...
			/**
			 * Checking for facet data exists or not.
			 * @param {object} oFacet - The Facet model
			 * @returns {boolean} true if annotation path is not of type line item and chart false otherwise
			 */
			getDataFieldCollectionEnabled: function(oFacet) {
				var sAnnotationPath = oFacet.Target && oFacet.Target["$AnnotationPath"];
				return (
					sAnnotationPath &&
					sAnnotationPath.indexOf("com.sap.vocabularies.UI.v1.LineItem") === -1 &&
					sAnnotationPath.indexOf("com.sap.vocabularies.UI.v1.Chart") === -1
				);
			},
			/**
			 * Method to get enabled expression for formElement.
			 *
			 * @function
			 * @name getFormElement
			 * @param {object} oDataField - DataPoint's Value
			 * @returns {boolean} - returns true if datafield type is neither DataFieldForAction nor DataFieldForIntentBasedNavigation and no hidden binding false otherwise
			 */
			getFormElement: function(oDataField) {
				return (
					oDataField["$Type"] !== "com.sap.vocabularies.UI.v1.DataFieldForAction" &&
					oDataField["$Type"] !== "com.sap.vocabularies.UI.v1.DataFieldForIntentBasedNavigation" &&
