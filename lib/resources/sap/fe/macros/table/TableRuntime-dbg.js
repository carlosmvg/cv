...
sap.ui.define(
	...
	function(JSONModel, CommonHelper, CommonUtils, FELibrary) {
		...
		var TableRuntime = {
			...
			setContexts: function(oTable, sModelName, sPrefix, sDeletablePath, oDraft, sCollection, sActionsMultiselectDisabled) {
				...
				for (var i = 0; i < aSelectedContexts.length; i++) {
					...
					if (sDeletablePath != "undefined") {
						if (sDeletablePath.indexOf("/") > -1) {
							//when deletable path is coming from navigation properties then we need to drill down to get the value
							aPaths = sDeletablePath.split("/");
							bDeletablePathValue = oSelectedContext.getObject();
							for (var j = 0; j < aPaths.length; j++) {
								bDeletablePathValue = bDeletablePathValue[aPaths[j]];
							}
						} else {
							bDeletablePathValue = oSelectedContext.getProperty(sDeletablePath);
						}
						if (bDeletablePathValue) {
				...
				if (aSelectedContexts.length > 1) {
					this.disableAction(oContextModel, aActionsMultiselectDisabled, sContextCollectionName, oDynamicActions);
				}
			...
			disableAction: function(oContextModel, aActionsMultiselectDisabled, sContextCollectionName, oDynamicActions) {
