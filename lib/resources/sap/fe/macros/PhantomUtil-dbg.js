...
sap.ui.define(
	...
	function(JSONModel, Log, ManagedObject, ResourceModel, XMLPreprocessor, SyncPromise, TraceInfo) {
		 ...
		 * @private
		 */
		function _getMetadataContext(oSettings, oNode, sAttributeName, oVisitor) {
			var oMetadataContext;
			if (oNode.hasAttribute(sAttributeName)) {
				...
				if (!oMetadataContext) {
					oMetadataContext = {
						model: "entitySet",
						path: oSettings.bindingContexts.entitySet.getPath(sAttributeValue)
					};
				}
		...
		function resolve(oMacroDefinition, oNode, oVisitor) {
			...
			if (oMacroDefinition.hasValidation) {
				...
				for (j = 0; j < aDefinitionContextsKeys.length; j++) {
					...
					var oMetadataContext = _getMetadataContext(oSettings, oNode, sAttributeName, oVisitor);
					if (oMetadataContext) {
