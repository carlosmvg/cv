sap.ui.define(["sap/ui/base/Object",
	...
	], function(BaseObject, SelectionVariant, Device, UIState, ControllerExtension, FeLogger, deepEqual, extend, deepExtend, FeError) {
	"use strict";
	var	sClassName = "AnalyticalListPage.controller.IappStateHandler";
...
`		@param {object} App data
		...
		*/
		...
		function _fnIsLoadDataOnAppLaunch() {
			var bDataLoad = false;
			if (oState.oSmartFilterbar.isCurrentVariantStandard()) {
				var bApplyAutomaticallyChangedByUser = oState.oSmartFilterbar.getSmartVariant().bExecuteOnSelectForStandardByUser;
				// null indicates not changed by user
				if (bApplyAutomaticallyChangedByUser !== null) {
					return false;
				}
				oState.oSmartFilterbar.checkSearchAllowed(oState);
				if (oState.oController.getView().getModel("_templPriv").getProperty("/alp/searchable")) {
					...
					if (bDataLoad) {
						...
						oState.oSmartFilterbar.getSmartVariant().bExecuteOnSelectForStandardViaXML = true;
					}
				}
			}
			return bDataLoad;
