sap.ui.define([
	...
	var SmartVisualFilterBar = HeaderContainer.extend("sap.suite.ui.generic.template.AnalyticalListPage.control.visualfilterbar.SmartVisualFilterBar", {
		metadata: {
			...
			properties: {
				...
				allFiltersAsInParameters: { type: "boolean", group: "Misc", defaultValue: false }
			},
			associations: {
	...
	*/
	...
	SmartVisualFilterBar.prototype._createHeaderItems = function (sPath, sType, oProperties) {
		...
		sId = this.getView().sId + "--" + StableIdHelper.getStableId({type: "VisualFilterBar", subType: "FilterItemContainer", sProperty: oFilterItem.getParentProperty()});
		var oCell = new VBox({
			id: sId,
	...
	*/
	...
	SmartVisualFilterBar.prototype._createTitleToolbar = function(props, filterItem) {
		...
		if (oInput) {
			...
			sId = this.getView().sId + "--" + StableIdHelper.getStableId({type: "VisualFilterBar", subType: "ValueHelpButton", sProperty: filterItem.getParentProperty()});
	...
	SmartVisualFilterBar.prototype._updateVisualFilterAria = function(oCell, oFilterItem, sOverlay) {
		...
		if (oFilterItem.getProperty("isMandatory")) {
			sParentProperty = FilterUtil.getPropertyNameDisplay(oFilterItem.getModel(), oFilterItem.getEntitySet(), sParentProperty, this.getModel("i18n"));
