sap.ui.define(["sap/ui/model/odata/AnnotationHelper",
	...
	"sap/ui/Global",
	"sap/ui/Device"
], function (AnnotationHelperModel, SmartField, FeLogger, formatMessage, deepExtend, encodeXML, RuntimeFormatters, StableIdHelper, testableHelper, Device) {
	"use strict";
...
*/
		...
		getVisibiltyBasedOnImportance: function(oVisible) {
			if (oVisible['com.sap.vocabularies.UI.v1.Importance'] && oVisible['com.sap.vocabularies.UI.v1.Importance'].hasOwnProperty("EnumMember")) {
				if (oVisible['com.sap.vocabularies.UI.v1.Importance'].EnumMember === 'com.sap.vocabularies.UI.v1.ImportanceType/High') {
					return true;
				} else if ((oVisible['com.sap.vocabularies.UI.v1.Importance'].EnumMember === 'com.sap.vocabularies.UI.v1.ImportanceType/Medium') && (Device.Device.system.tablet || Device.Device.system.desktop)) {
					return true;
				} else if (Device.Device.system.desktop) {
					return true;
				} else {
					return false;
				}
		...
		getImportenceForTableColumns: function(oVisible) {
			if (oVisible['com.sap.vocabularies.UI.v1.Importance'] && oVisible['com.sap.vocabularies.UI.v1.Importance'].hasOwnProperty("EnumMember")) {
				if (oVisible['com.sap.vocabularies.UI.v1.Importance'].EnumMember === 'com.sap.vocabularies.UI.v1.ImportanceType/Low') {
					return "Low";
				} else if (oVisible['com.sap.vocabularies.UI.v1.Importance'].EnumMember === 'com.sap.vocabularies.UI.v1.ImportanceType/Medium') {
					return "Medium";
				}
			}
			return "High";
		...
		isCreateWithParameter: function (oFacet, oSections) {
			var oSettings = oSections[oAnnotationHelper.getStableIdPartFromFacet(oFacet)];
			return !!(oSettings && oSettings.tableSettings && oSettings.tableSettings.createWithParameterDialog && oSettings.tableSettings.createWithParameterDialog.fields && oSettings.tableSettings.createWithParameterDialog.fields);
		},
		...
		getRepeatIndex: function (oValue) {
			if (oValue && oValue.getPath()) {
				...
				oLogger.error("Annotation Helper: Unable to get index.");
			}
		},
		...
		isRelatedEntityCreatable: function (oInterface, oSourceEntitySet, oRelatedEntitySet, aSubPages, oFacet, oSections, bIsDraftEnabled) {
			...
			if (oAnnotationHelper.hasSubObjectPage(oRelatedEntitySet, aSubPages) || (oSections && oAnnotationHelper.isInlineCreate(oFacet, oSections)) || (oSections && oAnnotationHelper.isCreateWithParameter(oFacet, oSections))) {
				if (bIsDraftEnabled) {
					result = "{= ${ui>/editable}}";
				} else {
					result = "{= !${ui>/editable}}";
