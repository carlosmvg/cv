...
sap.ui.define([
	...
	"sap/ui/Device"
], function(AnnotationHelperModel, AnnotationHelper, Device) {
	"use strict";
	 ...
	 */
	function getFCLPathExpression(oAppComponent, oTreeNode) {
	...
	function isEditButtonRequired(oInterface, mRestrictions, oEntitySet, bParameterEdit, bIsDraftEnabled, iViewLevel){
		return (iViewLevel === 1 || !bIsDraftEnabled) && AnnotationHelper.areBooleanRestrictionsValidAndPossible(oInterface, mRestrictions, oEntitySet, "Updatable", undefined, bParameterEdit);
	}
	isEditButtonRequired.requiresIContext = true;

	// Returns the visibility for the EDIT button on main object page
	function getEditActionButtonVisibility(oInterface, mRestrictions, oEntitySet, oAppComponent, oTreeNode) {
		...
		if (oTreeNode.isDraft && oAppComponent.getShowDraftToggle()){ // Exclude the situation that there is already a draft for this user (in this case the CONTINUE_EDITING-button would be shown).
		...
		var sFCLNonDraftExpression = getFCLPathExpression(oAppComponent, oTreeNode);
		...
		return fnCombineByAnd(sGeneralExpression, sToggleExpression, sFCLNonDraftExpression, sPathExpression);
	}
	...
	function getDeleteActionButtonVisibility(oInterface, mRestrictions, oEntitySet, oAppComponent, oTreeNode){
		...
		return fnCombineByAnd(sGeneralExpression, sFCLNonDraftExpression, sPathExpression);
	}
