sap.ui.define(["sap/ui/core/mvc/OverrideExecution",
			  ...
			  ], function(OverrideExecution, TemplateAssembler, ControllerImplementation, controllerFrameworkExtensions, testableHelper, semanticDateRangeTypeHelper, staticChecksHelper, preparationHelper, deepExtend, FeError) {
	"use strict";
				...
				if (oLrSettings.quickVariantSelectionX) {
					...
					for (var sKey in oVariants) {
						...
						if (!oVariants[sKey].isSmartChart) {
							...
							var oLineItem = preparationHelper.getLineItemFromVariant(oMetaModel, oMetaModel.getODataEntitySet(sEntitySet).entityType, oVariants[sKey].variant);
