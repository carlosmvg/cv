sap.ui.define([
	...
	var sAppState = "sap-iapp-state"; // this appState will carry the state of this view (the root view)
	...
	function getMethods(oState, oController, oTemplateUtils) {
		...
		function fnStoreCurrentAppStateAndAdjustURL() {
			...
			try {
				...
				oLogger.error("ListReport.fnStoreCurrentAppStateAndAdjustURL: " + err);
				return;
			}
			...
			oStoringInformation.promise.fail(function(oError){
				oLogger.error("ListReport.fnStoreCurrentAppStateAndAdjustURL: Error when persisting appState" + oError);
			});
			...
			} else if (oTemplateUtils.oComponentUtils.isComponentActive()) { // if the appstateKey really represents a new state and the view is still active set it to hash
				fnLogInfo("Call navigateByExchangingQueryParam", {
					appStateKey: oStoringInformation.appStateKey
				});
				oTemplateUtils.oServices.oApplication.navigateByExchangingQueryParam(sAppState, oStoringInformation.appStateKey);
			} else { // if the appstateKey really represents a new state and the view is no longer active -> at least ensure that we keep the information about last realized state
		...
		function getUrlParameterInfo(){
			return oAppStateIsSetPromise.then(function(){
				...
				if (oRealizedAppState.appStateKey){
					oRet[sAppState] = oRealizedAppState.appStateKey;
				}
				return oRet;
			});
		}
		...
		*/
		...
		function fnAdaptToAppState(oAppData, oURLParameters, sNavType){
			...
			if (sNavType !== sap.ui.generic.app.navigation.service.NavType.iAppState && oAppData.presentationVariant !== undefined) {
				var oAppPresentationVariant = JSON.parse(oAppData.presentationVariant);
				var aSortOrder = oAppPresentationVariant && oAppPresentationVariant.SortOrder;
				oTemplateUtils.oCommonUtils.setControlSortOrder(oState, aSortOrder);
			}
				...
				if (!oStoringInformation || oStoringInformation.appStateKey !== sAppStateKey){
					...
					if (!oState.oWorklistData.bWorkListEnabled) {
						...
						if (!bHasOnlyDefaults || oState.oSmartFilterbar.isCurrentVariantStandard()) {
							...
							fnApplySelectionVariantToSFB(bHasOnlyDefaults ? fnMergeDefaultWithSFBVariant(oStartupObject.selectionVariant, oSFBSelectionVariant) : oStartupObject.selectionVariant, oRealizedAppState, sSelectionVariant, true);
		...
		function isStateChange(mAppStates){
			sAppStateKeyInUrl = mAppStates[sAppState] || "";
			if (oStoringInformation){
				if (oStoringInformation.appStateKey !== sAppStateKeyInUrl){
					oLogger.error("ListReport.fnStoreCurrentAppStateAndAdjustURL: Got AppstateKey " + sAppStateKeyInUrl + " expected " + oStoringInformation.appStateKey);
					return false;
				}
