...
sap.ui.define([
	...
	"sap/suite/ui/generic/template/listTemplates/semanticDateRangeTypeHelper"
		        ...
		        WorklistHandler, ShareUtils, FeLogger, ObjectPath, StableIdHelper, deepExtend, CreateWithDialogHandler, SemanticDateRangeHelper) {
	"use strict";
	...
	var oMethods = {
		getMethods: function(oViewProxy, oTemplateUtils, oController) {
			...
			function addEntry(sVariantKey) {
				...
				if (oController.getOwnerComponent().getCreateWithParameterDialog()) {
					//oTemplateUtils.oCommonUtils.executeIfControlReady(oState.oCreateWithDialogHandler.createWithDialog, sIdForCreateButton);
			...
			return {
				...
				handlers: {
					...
					onDataReceived: function(oEvent) {
						...
						oTemplatePrivateModel.setProperty("/generic/bDataAreShownInTable", true);
					},
