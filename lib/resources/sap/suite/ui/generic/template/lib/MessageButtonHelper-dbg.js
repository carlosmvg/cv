			...
			oItemBinding.attachChange(function () {
				...
				if (iCount > 0) {
					...
					for (var i = 0; i < aItems.length; i++) {
						...
						} else if (aItems[i].getType() === "Information") {
					...
					if (iErrorCount > 0) {
						oMessageInfo = {
							iSeverityMessageCount: iErrorCount,
							messageSeverity: "Negative",
							icon: "sap-icon://message-error"
						};
					} else if (iWarningCount > 0) {
						oMessageInfo = {
							iSeverityMessageCount: iWarningCount,
							messageSeverity: "Critical",
							icon: "sap-icon://message-warning"
						};
					} else if (iInfoCount > 0) {
						oMessageInfo = {
							iSeverityMessageCount: iInfoCount,
							messageSeverity: "Emphasized",
							icon: "sap-icon://message-information"
						};
					} else {
						oMessageInfo = {
							iSeverityMessageCount: iSuccessCount,
				...
				oTemplatePrivate.setProperty("/generic/messageCount", oMessageInfo.iSeverityMessageCount);
