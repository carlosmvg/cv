...
sap.ui.define([
	...
	CommonMethods, FeLogger, extend, isPlainObject, FeError) {
	"use strict";
	...
	function getMethods(oAppComponent, oAppId) {
		...
		var	oTemplateContract = { // template contract object which is used for data interchange between framework classes
			...
			ghostapp: !!(oAppComponent.getManifestEntry("sap.ui.generic.app").settings || {}).ghostapp, // if this is true, this app should not interfere with the environment
		...
		//     This property is set in function fnCreateTemplateComponent in class NavigationController.
	...
	return UIComponent.extend("sap.suite.ui.generic.template.lib.AppComponent", {
		metadata: {
			...
			properties: {
				// ghostapp: a boolean property which is not used publically but only used to identify the ghostapp
				forceGlobalRefresh: {
					type: "boolean",
					defaultValue: true
				},
