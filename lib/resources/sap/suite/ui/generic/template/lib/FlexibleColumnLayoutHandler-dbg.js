	...
	function(BaseObject, FlexibleColumnLayoutSemanticHelper, fioriLibrary, extend) {
		...
		function getMethods(oFlexibleColumnLayout, oNavigationControllerProxy) {
				...
				if (oCurrentIdentity && sCurrentLayout !== oUiState.layout){
					...
					aActiveComponents.forEach(function(sComponentId){
						...
						if (sLastLayout !== sApplicapleLayout){
							...
							if (sLastLayout){
								oRegistryEntry.utils.layoutChanged(sLastLayout, sApplicapleLayout);                        
							}
						}
					});
				}
			...
			function onActionButtonPressed(oTreeNode, sActionName, bCloseColumn) {
				...
				if (bCloseColumn) {
					oTemplateContract.oDataLossHandler.performIfNoDataLoss(fnProcessFunction, Function.prototype);
				} else {
					fnProcessFunction();
				}
			}
