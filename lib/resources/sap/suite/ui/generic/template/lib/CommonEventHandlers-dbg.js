sap.ui.define(["sap/ui/base/Object",
	...
	testableHelper, JSONModel, coreLibrary, AnnotationHelper, modelHelper, SelectionVariant, Controller, FeLogger, MessageUtils, extend, isEmptyObject) {
	...
	function getMethods(oController, oComponentUtils, oServices, oCommonUtils) {
		...
		function getSmartTableFromEvent(oEvent){
			...
			return oCommonUtils.isMTable(oTable) ? oTable.getParent() : oTable;
		}
        ...
        function storeObjectPageNavigationRelatedInformation(oEventSource, oState) {
			...
			if (!bIsAnalyticalTbl) { // up/down navigation is not enabled in the analytical table scenario
				...
				if (oTable && iIdx !== -1 && aCurrContexts.length > 0) {
					...
					var fnPaginate = function(oContext){
						...
						if (!oVirtualItem){
							oVirtualItem = oEventSource.clone();
		...
		*/
		...
		function fnBuildSelectionVariantForNavigation(oOutbound, oLineContext, oPageContext, sFilterBarSelectionVariant){
			...
			var oPageContextObject = fnExtractPropertiesFromContextObject(oPageContext);
			var oLineContextObject = fnExtractPropertiesFromContextObject(oLineContext);
...
//			}
		...
		function fnMergeContextObjects(aContexts, oSmartFilterbarVariant) {
			...
			aContexts.forEach(function(oContext){
				var oProperties = fnExtractPropertiesFromContextObject(oContext);
				for (var sPar in oProperties){
					if (aInputParams.indexOf(sPar) === -1){
						var vEntry = oProperties[sPar];
						if (vEntry){
							oSelectionVariant.addSelectOption(sPar, "I", "EQ",
								typeof vEntry === "object" ? JSON.stringify(vEntry) : vEntry.toString()
							);
						}
					}
				}
		...
		function onDataFieldForIntentBasedNavigationSelectedContext(oSource, oCustomData, aContexts, oState) {
			...
			if (aContexts.length <= 1) {
				...
				var fnHandleMultiContextIBN = function() {
					//oState.oSmartFilterbar is undefined when the source navigation is from a table in the object oage
					var oSmartFilterbarVariant = oState.oSmartFilterbar && oState.oSmartFilterbar.getUiState().getSelectionVariant();
		 ...
		 * @return Bool
		 */
		function hasContentIdReferencing() {
		...
		function addEntry(oEventSource, bSuppressNavigation, oSmartFilterBar, oPredefinedValues, bIgnoreTableRefresh, bCreateWithDialogEnabled) {
			...
			var fnCRUDManagerCall = function(oPredefined){
				var oRet = oServices.oCRUDManager.addEntry(oTable, oPredefined, hasContentIdReferencing()).then(function(oNewContext){
