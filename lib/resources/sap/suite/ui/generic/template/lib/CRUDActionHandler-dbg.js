 ...
 */
 ...
 sap.ui.define(["sap/ui/base/Object", "sap/ui/model/Filter", "sap/ui/model/FilterOperator", "sap/base/util/extend", "sap/ui/generic/app/util/MessageUtil", 	"sap/ui/model/json/JSONModel"
], function(BaseObject, Filter, FilterOperator, extend, GenericMessageUtil, JSONModel) {
	"use strict";
	...
	function getMethods(oTemplateContract, oController, oCommonUtils) {
		...
		// sCRUDAction: Activate, Delete
		function getConfiguredPopoverIfNeeded(iSituation, sCRUDAction, aMessages){
			...
			return new Promise(function (fnResolve) {
				...
				oCommonUtils.getDialogFragmentAsync("sap.suite.ui.generic.template.fragments.MessageInfluencingCRUDAction", {
					...
					oLocalModel.setProperty("/backbtnvisibility", false);
					oLocalModel.setProperty("/title", oRet.getModel("i18n").getProperty("ST_DETAIL_TITLE_VALIDATION"));
					...
					if (iSituation === 3) {
						...
						bCreateMode ? oLocalModel.setProperty("/title", oRet.getModel("i18n").getProperty("ST_DETAIL_TITLE_CREATE")) : oLocalModel.setProperty("/title", oRet.getModel("i18n").getProperty("ST_DETAIL_TITLE_SAVE"));
						if (sCRUDAction === "Delete") {
							var sObjectTitle = (oRet.getModel("_templPriv").getProperty("/objectPage/headerInfo/objectTitle") || "").trim();
							var sObjectSubtitle = oRet.getModel("_templPriv").getProperty("/objectPage/headerInfo/objectSubtitle");
							var aParams, sTitle;	
							if (sObjectTitle) {
								if (sObjectSubtitle) {
									aParams = [" ", sObjectTitle, sObjectSubtitle];
									sTitle = oCommonUtils.getText("CONFIRMATION_POPUP_TITLE_DELETE_WITH_OBJECTINFO", aParams);
								} else {
									aParams = [sObjectTitle];
									sTitle = oCommonUtils.getText("CONFIRMATION_POPUP_TITLE_DELETE_WITH_OBJECTTITLE", aParams);
								}
							} else {
								sTitle = oCommonUtils.getText("CONFIRMATION_POPUP_TITLE_ST_GENERIC_DELETE_SELECTED");
							}
							oLocalModel.setProperty("/title", sTitle);
						}
						if (oItemBinding.filter(oErrorFilter).getLength() === 0) {
							oLocalModel.setProperty("/situation", 4);
							oLocalModel.setProperty("/CRUDAction", sCRUDAction);
						}
		...
		function fnRestartCRUDAction(fnOperationCallback, fnCancelCallback, sCRUDAction, aMessages){
			fnYes = fnOperationCallback;
			fnNo = fnCancelCallback;
			return getConfiguredPopoverIfNeeded(3, sCRUDAction, aMessages).then(function(oWarningDialog){
				if (oWarningDialog) {
		...
		function fnHandleCRUDScenario(iScenario, fnPerformOperation, fnCancel, sCRUDAction, aMessages) {
			if (iScenario === 4) {
				return fnRestartCRUDAction(fnPerformOperation, fnCancel, sCRUDAction, aMessages);
			} else {
