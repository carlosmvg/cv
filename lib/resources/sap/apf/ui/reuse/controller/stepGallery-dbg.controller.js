...
sap.ui.define([
	...
	'sap/apf/utils/trace',
	'sap/m/MessageToast'
], function(UiUtilsHelper, trace, MessageToast) {
	'use strict';
	...
	sap.ui.controller("sap.apf.ui.reuse.controller.stepGallery", {
		...
		*/
		onStepPress : function(sId, oRepresentationType, sTitle) {
			trace.logCall("stepGallery.onStepPress -- adds an active step, id=", sId, " ************************************** ");
			var oController = this;
			var self = this;
			var previousSteplength = oController.oCoreApi.getSteps().length;
			...
			this.oCoreApi.checkAddStep(sId).done(function(bCanStepBeAdded, oMessageObject) {
				...
				var currentSteplength = oController.oCoreApi.getSteps().length;
				if(previousSteplength < currentSteplength) {
					MessageToast.show(self.oCoreApi.getTextNotHtmlEncoded("msgToast" , [ "'" + sTitle + "'" ]));
				}
			});
