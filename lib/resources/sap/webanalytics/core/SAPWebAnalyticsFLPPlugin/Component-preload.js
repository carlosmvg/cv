	...
	var SWAComponent = Component.extend("sap.webanalytics.core.SAPWebAnalyticsFLPPlugin.Component", {
		...
		loadSWAObject: function(SWA_PUB_TOKEN, TRACKING_URL, LOGGING, usr) {
			window.swa = {
				...
				owner: usr,
				custom1: {ref: this.getCustomAttributes(usr)[0]},
				custom2: {ref: this.getCustomAttributes(usr)[1]},
				custom3: {ref: this.getCustomAttributes(usr)[2]},
				custom4: {ref: this.getCustomAttributes(usr)[3]},
				custom5: {ref: this.getCustomAttributes(usr)[4] }
			};
