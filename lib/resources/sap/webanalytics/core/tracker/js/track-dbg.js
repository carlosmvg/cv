...
var i18nValues = [],defaultConsentValues = [];
...
function addBanner () {
    ...
    swa.jQuery('body').append('<style>.swa_banner {font-family:Arial,Helvetica,sans-serif;width: 100%;height: auto;bottom:0;min-height: 20px;background-color: #FFFFFF;z-index: 9999;position: absolute;left: 0;font-size:80%; padding: 10px 10px 10px 10px;}.swa_banner p {display: inline;position: static;left: 0px; word-spacing:1px;line-height: 17px;}.swa_banner img{vertical-align:top;}.swa_banner form {display: inline;position: absolute;right: 0px;margin-right: 25px;}#buttonsDiv{float: right;margin-right:30px;padding: 5px 5px 0 0;}.bannerButton {margin-right:15px;background-color: #0066ff;border: none;color: white;padding:5px 5px;font-size: 12px;cursor: pointer;width: 60px;border-radius:4px;}.bannerButton:hover{background-color: #075caf;} button#yesButton:disabled { opacity: 0.3; background-color:#0066ff}</style><div id="swa_banner" class="swa_banner"><p><strong>'+swa.consentOptInText+'</strong></p></br><hr><div id="buttonsDiv" ><button type="button" id="yesButton" class = "bannerButton" title='+swa.consentAllowButton+' >'+swa.consentAllowButton+'</button><button  title='+swa.consentDenyButton+' type="button" class = "bannerButton" id="noButton" >'+swa.consentDenyButton+'</button><a href="#" title='+swa.consentCloseButton+' style="margin-top:5px; margin-left:20px;" ><img id="closeButton"  src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQAQMAAAAlPW0iAAAABlBMVEUAAACIiIhaeOqmAAAAAXRSTlMAQObYZgAAAC9JREFUaN5jYGBgEOBgsJBhqLFjsKthkP/BwP+Bgf0BCAEZQC5QECgFVABUxsAAAOcxCbuaDAybAAAAAElFTkSuQmCC" /></a></div>');         
		else	
	swa.jQuery('body').append('<style>.swa_banner {font-family:Arial,Helvetica,sans-serif;width: 100%;height: auto;top:0;min-height: 20px;background-color: #FFFFFF;z-index: 9999;position: absolute;left: 0;font-size:80%; padding: 10px 10px 10px 10px;}.swa_banner p {display: inline;position: static;left: 0px; word-spacing:1px;line-height: 17px;}.swa_banner img{vertical-align:top;}.swa_banner form {display: inline;position: absolute;right: 0px;margin-right: 25px;}#buttonsDiv{float: right;margin-right:30px;padding: 5px 5px 0 0;}.bannerButton {margin-right:15px;background-color: #0066ff;border: none;color: white;padding:5px 5px;font-size: 12px;cursor: pointer;width: 60px;border-radius:4px;}.bannerButton:hover{background-color: #075caf;} button#yesButton:disabled { opacity: 0.3; background-color:#0066ff}</style><div id="swa_banner" class="swa_banner"><p><strong>'+swa.consentOptInText+'</strong></p></br><hr><div id="buttonsDiv" ><button type="button" id="yesButton" title='+swa.consentAllowButton+' class = "bannerButton" >'+swa.consentAllowButton+'</button><button type="button" class = "bannerButton" title='+swa.consentDenyButton+' id="noButton" >'+swa.consentDenyButton+'</button><a href="#" title='+swa.consentCloseButton+' style="margin-top:5px; margin-left:20px;" ><img id="closeButton"  src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQAQMAAAAlPW0iAAAABlBMVEUAAACIiIhaeOqmAAAAAXRSTlMAQObYZgAAAC9JREFUaN5jYGBgEOBgsJBhqLFjsKthkP/BwP+Bgf0BCAEZQC5QECgFVABUxsAAAOcxCbuaDAybAAAAAElFTkSuQmCC" /></a></div>');	
...
function preferencesDone(pubTokenAttributes) {
	...
	if(!isLanguageConsent && (consentTextPresent) ){
		...
		swa.consentCloseButton = i18nValues[4];
	}
	...
	if(!siteProperties.visitorUUIDAnonymity && siteProperties.internalApplicationUser){
		swa.cookieTimeoutForVisitor = (siteProperties.cookieTimeOut <= siteProperties.globalCookieTimeout*86400) ? siteProperties.cookieTimeOut : siteProperties.globalCookieTimeout*86400;
	}
	else{
	...
	if(swa.isConsentGiven){
		// check if cookies are present if present do not create just change the
		// _swa_tracking_consent cookie value if changed
	...
	swaCookies = getSWACookieValue(swa.parentCookies);
		...
		for (i = 0; i < swaCookies.length; i++) {  
			if(swaCookies[i].indexOf("_swa_tracking_consent")==0 || swaCookies[i].indexOf("_swa_id")==0){
				validUntil = Date.now() + swa.cookieTimeoutForVisitor * 1000;
				expiryDate = new Date(parseInt(validUntil, 10));
             document.cookie = swaCookies[i]+"; expires=" + expiryDate.toUTCString()+"; path=/";
   document.cookie = "trackingConsentExpiry="+expiryDate.toUTCString();
			}
			else if(swaCookies[i].indexOf("_swa_ref")==0)
				{				
				validUntil = Date.now() + swa.referralCookieTimeout * 1000;
				expiryDate = new Date(parseInt(validUntil, 10));
				//expiry date set to 30 minutes for session,referrer cookies like parent cookies
				document.cookie = swaCookies[i]+"; expires=" + expiryDate.toUTCString()+"; path=/";
				}
			else if(swaCookies[i].indexOf("_swa_ses")==0){
				validUntil = Date.now() + 1800 * 1000;
				expiryDate = new Date(parseInt(validUntil, 10));
				//expiry date set to 30 minutes for session,referrer cookies like parent cookies
				document.cookie = swaCookies[i]+"; expires=" + expiryDate.toUTCString()+"; path=/";
			}
  ...
  document.cookie = swaCookies[i]+"; expires=" + swa.getCookieValue("trackingConsentExpiry")+"; path=/";
				}
			}
		}
	}
	
	if(!siteProperties.visitorUUIDAnonymity && !siteProperties.bannerEnabled && !siteProperties.globalBannerEnabled && siteProperties.internalApplicationUser && !siteProperties.hasPersonalData)
...
if(!swa.isConsentGiven)
	...
	_paq.push(["setCustomRequestProcessing", function (data) {
		...
		// the referrer needs change by a defined swa.urlFormatterCallback
		// function.
		...
		if (data) {
			...
			if ((window) && (window.location)) {
				...
				// console.log("setCustomRequestProcessing - currentURL: " +
				// currentURL);
				// If swa.urlFormatterCallback is defined, call it with current
				// URL and signal it's the URL of the page
				if (typeof swa.urlFormatterCallback === "function") {
					// console.log("setCustomRequestProcessing - calling
					// swa.urlFormatterCallback for page URL");
					currentURL = swa.urlFormatterCallback(currentURL, "page");
					// console.log("setCustomRequestProcessing - page URL
					// received from callback: " + currentURL);
				}
				// Check if param "url" exists and if it has the same value as
				// the current (or changed) URL
				if ("url" in reportedParams) {
					// console.log("setCustomRequestProcessing - Piwik's
					// reported URL: " + reportedParams.url);
					if (reportedParams.url != currentURL) {
						// console.log("setCustomRequestProcessing - mismatch:
						// Reported URL does not match current URL!!!");
						// Obviously Piwik is wrong about the current URL. So
						// first we update current URL in Piwik.
			...
			if (swa.referrerOfTop) {
				// console.log("setCustomRequestProcessing - swa.referrerOfTop
				// is true");
				// Get the current referrer the same way as Piwik tries to get
				// it
				try {
					currentReferrer = window.top.document.referrer;
					// console.log("setCustomRequestProcessing -
					// window.top.document.referrer: " + currentReferrer);
				} catch (e) {
					// Ignore
				}
				if (currentReferrer === "") {
			...
			if ((currentReferrer === "") && (document)) {
				...
				// console.log("setCustomRequestProcessing - document.referrer:
				// " + currentReferrer);
			}
			// console.log("setCustomRequestProcessing - currentReferrer: " +
			// currentReferrer);
			// If swa.urlFormatterCallback is defined, call it with current
			// referrer and signal it's the URL of the referrer
			if (typeof swa.urlFormatterCallback === "function") {
				// console.log("setCustomRequestProcessing - calling
				// swa.urlFormatterCallback for referrer URL");
				currentReferrer = swa.urlFormatterCallback(currentReferrer, "referrer");
				// console.log("setCustomRequestProcessing - referrer URL
				// received from callback: " + currentReferrer);
			}
			// Check if param "urlref" exists and if it has the same value as
			// the current (or changed) referrer
			if ("urlref" in reportedParams) {
				// console.log("setCustomRequestProcessing - Piwik's reported
				// referrer: " + reportedParams.urlref);
				if (reportedParams.urlref != currentReferrer) {
					// console.log("setCustomRequestProcessing - mismatch:
					// Reported referrer does not match current referrer!!!");
					// Obviously Piwik is wrong about the current referrer. So
					// first we update current referrer in Piwik.
					if (typeof Piwik !== "undefined")
			...
			// Check page title
			if (document) {
				// Note: This also works if document.title is dynamically
				// changed
				...
				if (titleTags.length > 0) {
					...
					// Check if param "pageTitle" exists and if it has the same
					// value as the current page title
					if ("pageTitle" in reportedParams) {
						// console.log("setCustomRequestProcessing - Piwik's
						// reported page title: " + reportedParams.pageTitle);
						if (reportedParams.pageTitle != currentPageTitle) {
							// Obviously Piwik is wrong about the current page
							// title, so first we update current title in Piwik.
							if (typeof Piwik !== "undefined")
								Piwik.getAsyncTracker().setDocumentTitle(currentPageTitle);
							// console.log("setCustomRequestProcessing -
							// mismatch: Reported page title does not match
							// current page title!!!");
							// Second, we correct the page title in the current
							// event
							reportedParams.pageTitle = currentPageTitle;
							// And we signal that query string needs to be
							// rebuild
							rebuildData = true;
						}
					}
			...
			// If visitor anonymity if OFF and visitor cookie timeout is set as
			// session, reset visitor for every session
			if(!siteProperties.visitorUUIDAnonymity && swa.setAsSession && reportedParams["_idvc"] > "1"){
				resetVisitor(); // Create a new visitor
				rebuildData = true;
				reportedParams._idvc = "1"; // Setting current event's visit
											// count to 1
			}
			...
			if (typeof swa.dataAnonymiseCallback === "function") {
				...
				// If customer anonymized any properties, update them to
				// parameter list
			...
			// If we did any corrections to the parameters, we need to rebuild
			// the query string
			if (rebuildData === true) {
...
function loadSWAPiwikPlugin() {
	...
	loadPlugin(function () {
		...
		// Trigger callback for "everything for tracking is loaded now - tracker
		// ready" (if it exists).
		// Check if swa object has a function in property "trackerReadyCallback"
		// - if yes, call it.
...
function addPluginEventHandlers(){
	...
	if (swa.trackFrames === true) {
		...
		try {
			...
			// Accessing the contents of an iframe that is not from the same
			// origin (cross-origin frame) throws a SecurityError.
			// See https://en.wikipedia.org/wiki/Same-origin_policy
			// We cannot register tracking event handlers to cross-origin
			// frames.
		}
	});
	}	
	...
	if (swa.hotkeysEnabled && (e.ctrlKey || e.altKey)
		&& (e.keyCode !== 17 && e.keyCode !== 18)) { // Ensure key is not
														// ctrl or alt
...
function addPluginContextToSWA(){
	...
	swa.plugin = {
		...
		// The event function is called if a custom event has manually been
		// triggered that should be logged.
		event: function() {
		...
		_getCommons: function() {
			...
			for(var i=0;i<10;i++){
				var customid='custom'+String(i+1);         // Get name of
															// costom field -
															// "customX"
				var func=swa.plugin._getCustomvalues(i+1); // Get array
															// [swa.customX.ref,swa.customX.params,swa.customX.isStatic]
				if (func===null)                           // If nothing
															// declared for
															// swa.customX, go
															// to next loop
															// iteration
				...
				if (typeof(func[0])==="function") {
					// ref passed is a function, we pass it on in the result
					// array.
					// result[customid] = func[0];
					result[customid] = func[0].apply(null, func[1]);
				} else {
					// Only try to convert customX.ref to a function, if
					// customX.isStatic is false or not set
					if (!func[2]) {
						// If ref is not a function, check if window scope has a
						// function with the name provided
						var fn = window[func[0]];
						if (typeof fn === "function") {
							result[customid] = fn.apply(null, func[1]);
						} else {
							// We got neither a function, nor the name of a
							// function in scope windows, so just pass on
							// whatever it is (e.g. static string).
							result[customid] = func[0];
						}
					} else {
						// If customX.isStatic is true, don't try to convert
						// func[0] to a function, but just pass it on
						result[customid] = func[0];
					}
				}
			}
...
function getEventMapper() {
	...
	mapper.localTimeHour = "h"; // localTime is handled by h,m,s values of
								// client
...
swa.disable = function() {
	...
	if(window.cXMLHttpRequest)
		window.cXMLHttpRequest.enableTracking = false; // Disable XHR Requests'
														// tracking
	if(isHeartBeatEnabled){
		disableHeartbeat();
	}
...
swa.trackCustomEvent = function (eventType, eventValue) {
	...
	if ((Array.isArray(arguments[1]) && arguments[1].length > 0) || (Array.isArray(arguments[2]) && arguments[2].length > 0)) {
		if ((swa.loggingEnabled) && (swa.customEventsEnabled) && (typeof Piwik !== "undefined")) {
			...
			// Add the array "additionalStringValues" to the array
			// "customEventAddValues" of our piwik plugin.
			// When processing the custom event in the plugin, we will add the
			// additional values to the request.
			swa.plugin.customEventAddValues.push(additionalStringValues);

			// Add the array "additionalIntValues" to the array
			// "customEventIntAddValues" of our piwik plugin.
			// When processing the custom event in the plugin, we will add the
			// additional values to the request.
...
swa.deleteCookies = function () {
	if (typeof Piwik !== "undefined")
		Piwik.getAsyncTracker().deleteCookies(); // This deletes Piwik set
													// cookies
...
function loadPromise(callback) {
	...
	swa._addScript("https://sapui5.hana.ondemand.com/resources/sap/ui/thirdparty/es6-promise.js", function () {
		callback();
	});
}
...
function loadJQuery(callback) {
    //Load UI5 jquery
	swa._addScript("https://sapui5.hana.ondemand.com/resources/sap/ui/thirdparty/jquery.js", function () {
...
swa.showConsentFormOnDemand = function(){
	...
	if(  swa.loggingEnabled  ){
		...
		if (swa.jQuery('div[swa_banner=1]').length === 0) {
            if(swa.consentStyle =="bannerbottom")
			swa.jQuery('body').append('<style>.swa_banner {font-family:Arial,Helvetica,sans-serif;width: 100%;height: auto; bottom:0;min-height: 20px;background-color: #FFFFFF;z-index: 9999;position: absolute;left: 0;font-size:80%; padding: 10px 10px 10px 10px;}.swa_banner p {display: inline;position: static;left: 0px; word-spacing:1px;line-height: 17px;}.swa_banner img{vertical-align:top;}.swa_banner form {display: inline;position: absolute;right: 0px;margin-right: 25px;}#buttonsDiv{float: right;margin-right:30px;padding: 5px 5px 0 0;}.bannerButton {margin-right:15px;background-color: #0066ff;border: none;color: white;padding:5px 5px;font-size: 12px;cursor: pointer;width: 60px;border-radius:4px;}.bannerButton:hover{background-color: #075caf;} button#yesButton:disabled { opacity: 0.3; background-color:#0066ff}</style><div id="swa_banner" class="swa_banner"><p><strong>'+swa.consentOptInText+'</strong></p></br><hr><div id="buttonsDiv" ><button type="button" id="yesButton" class = "bannerButton" >'+swa.consentAllowButton+'</button><button type="button" class = "bannerButton" id="noButton" >'+swa.consentDenyButton+'</button><a href="#" title='+swa.consentCloseButton+' style="margin-top:5px; margin-left:20px;" ><img id="closeButton"  src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQAQMAAAAlPW0iAAAABlBMVEUAAACIiIhaeOqmAAAAAXRSTlMAQObYZgAAAC9JREFUaN5jYGBgEOBgsJBhqLFjsKthkP/BwP+Bgf0BCAEZQC5QECgFVABUxsAAAOcxCbuaDAybAAAAAElFTkSuQmCC" /></a></div>');          
...
function loadPiwikUrl(){
	if(window.sap.ui !== undefined && parseFloat(sap.ui.version) > 1.80){
...
var isUI5 = false;	
var i18nurl = "";	
if ((typeof swa.consentOptInText !== "undefined" || typeof swa.consentOptOutText !== "undefined") && count <1)
{
consentTextPresent =  true;
}
count ++;
if(window.sap.ui !== undefined && parseFloat(sap.ui.version) > 1.80){
i18nurl = sap.ui.require.toUrl("sap/webanalytics/core/SAPWebAnalyticsFLPPlugin/i18n/i18n.properties");
swa.jQuery.sap.require("jquery.sap.resources");		
var oBundle = swa.jQuery.sap.resources({url : i18nurl, locale: swa.consentFormLanguage,fallbackLocale: "en"});
isUI5 = true;
getTranslatedText(oBundle,isUI5);
}else{
	i18nurl = swa.baseUrl+"js/i18n/i18n"+((swa.consentFormLanguage === "" ||swa.consentFormLanguage === undefined )?"":"_"+swa.consentFormLanguage)+".json";
...
function getTranslatedText(oBundle,isUI5){
	...
	i18nValues[0] = isUI5?oBundle.getText('CONSENT_OPT_IN_TEXT'):oBundle.CONSENT_OPT_IN_TEXT;	
	i18nValues[1] = isUI5?oBundle.getText('CONSENT_OPT_OUT_TEXT'):oBundle.CONSENT_OPT_OUT_TEXT;
	i18nValues[2] = isUI5?oBundle.getText('CONSENT_BUTTON_ALLOW'):oBundle.CONSENT_BUTTON_ALLOW;
	i18nValues[3] = isUI5?oBundle.getText('CONSENT_BUTTON_DENY'):oBundle.CONSENT_BUTTON_DENY;
	i18nValues[4] = isUI5?oBundle.getText('CONSENT_BUTTON_STOP_TRACKING'):oBundle.CONSENT_BUTTON_STOP_TRACKING;
	i18nValues[5] = isUI5?oBundle.getText('CONSENT_TOOLTIP_CLOSE'):oBundle.CONSENT_TOOLTIP_CLOSE;
	}
	if (typeof swa.consentOptInText === "undefined")
		swa.consentOptInText = isUI5?oBundle.getText('CONSENT_OPT_IN_TEXT'):oBundle.CONSENT_OPT_IN_TEXT;	
if (typeof swa.consentOptOutText === "undefined")
		swa.consentOptOutText = isUI5?oBundle.getText('CONSENT_OPT_OUT_TEXT'):oBundle.CONSENT_OPT_OUT_TEXT;	    
		swa.consentAllowButton = isUI5?oBundle.getText('CONSENT_BUTTON_ALLOW'):oBundle.CONSENT_BUTTON_ALLOW;
   swa.consentDenyButton =isUI5? oBundle.getText('CONSENT_BUTTON_DENY'):oBundle.CONSENT_BUTTON_DENY;
  swa.consentStopButton = isUI5?oBundle.getText('CONSENT_BUTTON_STOP_TRACKING'):oBundle.CONSENT_BUTTON_STOP_TRACKING;
   swa.consentCloseButton = isUI5?oBundle.getText('CONSENT_TOOLTIP_CLOSE'):oBundle.CONSENT_TOOLTIP_CLOSE;       
}
function fallback(){
	if(!(window.sap.ui !== undefined && parseFloat(sap.ui.version) > 1.80)){
...
function getDefaultTextForConsent(){
	if (typeof swa.consentOptInText === "undefined" || swa.consentOptOutText === "undefined")
		{
  swa.consentOptInText = defaultConsentValues[0];	
  swa.consentOptOutText = defaultConsentValues[1];		    
  swa.consentAllowButton = defaultConsentValues[2];
  swa.consentDenyButton =defaultConsentValues[3];
  swa.consentStopButton = defaultConsentValues[4];
  swa.consentCloseButton = defaultConsentValues[5];   	    
		}
}
})();
