...
sap.ui.define([
    ...
    return UIComponent.extend("sap.ushell.applications.PageComposer.Component", {
        ...
        init: function () {
            ...
            oUshellUtils.setPerformanceMark("FLPPage-manage.startInitialization", { bUseUniqueMark: true });
            ...
            // Overwrites the default sizeLimit of 100, mainly necessary for the TileSelector's lists
            this.getModel("PageRepository").setSizeLimit(1000);
            ...
            this.getModel("PageRepository").attachBatchRequestSent(function () {
                oUshellUtils.setPerformanceMark("FLPPage-manage.startLoadPage");

            });
            this.getModel("PageRepository").attachBatchRequestCompleted(function () {
                oUshellUtils.setPerformanceMark("FLPPage-manage.endLoadPage");
            });

            // set message model
            this.setModel(sap.ui.getCore().getMessageManager().getMessageModel(), "message");

            // set settings model to contain
            // - the editMode property, used to navigate directly into edit mode
            // - the deepLink property, used to determine if a deep link has been accessed
            this.setModel(new JSONModel({
                editMode: false,
                deepLink: History.getInstance().getDirection() === undefined
            }), "settings");
        ...
        setMetaModelData: function () {
            var oMetaModel = this.getModel("PageRepository").getMetaModel(),
                oMetaModelData = {
                    ...
                    translationSupported: !!oMetaModel.getODataEntitySet("translationSet")
                };
