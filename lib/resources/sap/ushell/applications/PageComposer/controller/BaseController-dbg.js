    ...
    return Controller.extend("sap.ushell.applications.PageComposer.controller.BaseController", {
        ...
        _openTileInfoPopover: function (oOpenByControl, oBindingContext) {
            ...
            if (!oRootView.oTileInfoPopover) {
                ...
                var oVizInstancePropertiesModel = new JSONModel({});
                if (oOpenByControl.isA("sap.ushell.ui.launchpad.VizInstance")) {
                    oVizInstancePropertiesModel.setProperty("/state", oOpenByControl.getState());
                    oVizInstancePropertiesModel.setProperty("/innerControlScope", oOpenByControl.getInnerControl().getScope());
                }
                oRootView.oTileInfoPopover.setModel(oVizInstancePropertiesModel, "vizInstance");
        ...
        _resetRolesModel: function (oSelectedRolesInfo) {
            ...
            if (typeof oSelectedRolesInfo === "undefined") {
                ...
                var sCount = oSelectedRolesInfo.selected.length.toString();
                if (!oSelectedRolesInfo.selected.length) {
                    sCount = this.getResourceBundle().getText("Message.NoRolesSelected");
                } else if (oSelectedRolesInfo.allSelected) {
                ...
                oRolesModel.setProperty("/selectedCountText", sCount); // to be displayed in the InfoToolbar
