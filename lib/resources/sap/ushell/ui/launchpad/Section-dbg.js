...
sap.ui.define([
    ...
    Section.prototype.init = function () {
        ...
        var fnAdjustAriaLabelToTitleChange = function () {
            this.oVBox.getDomRef().setAttribute("aria-label", this.getTitle());
        }.bind(this);
        ...
        this.oVBox.addEventDelegate({
            ...
            onAfterRendering: function () {
                ...
                oVBoxDomRef.setAttribute("aria-label", this.getTitle());
    ...
    Section.prototype.setShowSection = function (value, oEvent) {
        ...
        var sInvisibleMessage = resources.i18n.getText(value ? "Section.nowBeingShown" : "Section.nowBeingHidden");
        this._oInvisibleMessageInstance.announce(sInvisibleMessage, InvisibleMessageMode.Polite);
    };
    ...
    Section.prototype._onDragEnter = function (oEvent) {
        ...
        var oTargetGrid = oDragSession.getDropControl && oDragSession.getDropControl(); // not available in case of keyboard DnD
        var oSourceGrid = oDragSession.getDragControl().getParent();
        if (oTargetGrid && oTargetGrid.data("default") && !oSourceGrid.data("default")) {
     ...
     *
     * @param {integer} item Index or the visualization to focus
     * @private
     */
    Section.prototype._focusItem = function (item) {
        var oGrid = this.byId("innerGrid");
        window.setTimeout(function () {
            var nItems = oGrid.getItems().length;

            if (isNaN(item) || item < 0) { // focus the first item
                item = 0;
            } else if (item >= nItems) { // focus the last item
                item = nItems - 1;
            }
            if (oGrid.getDomRef() && nItems) {
                oGrid.focusItem(item);
            }
        }, 0);
    };
