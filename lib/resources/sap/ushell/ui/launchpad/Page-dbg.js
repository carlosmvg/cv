...
sap.ui.define([
    ...
    Page.prototype._focusOrMovetoNextVisualization = function (oInfo) {
        ...
        while (true) {
            ...
            if ((oInfo.visualization || aVisualizations.length) && (oSection.getShowSection() || oSection.getEditable())) {
                if (oInfo.prefIndex === undefined) {
                    iVizIndex = sDirection === "down" ? 0 : Math.max(aVisualizations.length - 1, 0);
                }

                if (oInfo.visualization) {
                    // We are in a drag&drop case.
                    // Do not move visualizations to the default section.
                    if (oSection.getDefault()) {
                        return;
                    }
