...
sap.ui.define([
    ...
    "sap/ushell/utils/testableHelper",
    "sap/ushell/Config",
    ...
    */
    ...
    function handleMessageEvent (oContainer, oMessage) {
        ...
        handlePostMessagesForPluginsPostLoading(oContainer, oMessage, oMessageStatus);
    }

    // Expose selected private static functions to unit tests
    /* eslint-disable */
    var handleMessageEvent = testableHelper.testableStatic(handleMessageEvent, "ApplicationContainer_handleMessageEvent");
    /* eslint-enable */
     ...
     *
     * @param {sap.ushell.components.container.ApplicationContainer} oContainer
     *   application container recieved the post message
     * @param {object} oMessage
     *   the post message recieved
        ...
        if (bPluginsStatusChecked === false) {
            ...
            if (oPluginManager) {
                ...
                if (sPluginPromiseState === "pending") {
                    ...
                    oPluginPromise.done(function () {
                        var oMsgObj;
                        bKeepMessagesForPlugins = false;
                        jQuery.sap.log.debug("Processing post messages queue after 'RendererExtensions' plugins loaded, queue size is: "
                        ...
                        for (var i = 0; i< arrMessagesForPlugins.length; i++) {
                            ...
                            try {
                                handleMessageEvent.call(oMsgObj.that, oMsgObj.oContainer, oMsgObj.oMessage);
                            } catch (ex) {
                                jQuery.sap.log.error(ex.message || ex, null, "sap.ushell.components.container.ApplicationContainer");
                            }
                        }
                        arrMessagesForPlugins = [];
                    });
    ...
    // Expose selected private static functions to unit tests
    /* eslint-disable */
    var handlePostMessagesForPluginsPostLoading = testableHelper.testableStatic(handlePostMessagesForPluginsPostLoading, "ApplicationContainer_handlePostMessagesForPluginsPostLoading");
    /* eslint-enable */
    ...
    // Expose selected private static functions to unit tests
    /* eslint-disable */
    var resetPluginsLoadIndications = testableHelper.testableStatic(resetPluginsLoadIndications, "ApplicationContainer_resetPluginsLoadIndications");
    /* eslint-enable */
    ...
    function render (oRenderManager, oContainer, sApplicationType, sUrl, sAdditionalInformation) {
        ...
        sUrl = this._adjustURLForIsolationOpeningWithoutURLTemplate(sUrl);

        //This block is relevant for "Starting WebGUI/WebDynpro apps with a POST" BLI since 1.68
        if (oContainer.getIframeWithPost() === true &&
            (utils.isApplicationTypeEmbeddedInIframe(sApplicationType) || oContainer.getIsPostCapability() === true)) {
        ...
        if (urlParams.get("sap-isolation-enabled") === "true") {
            ...
            if (sHash) {
                ...
                if (sParams && sParams.length >= 0) {
                    ...
                    oState2.save();
                    sUrl = sUrl.split("#")[0] + "&sap-intent-param=" + oState2.getKey() + sPlugins + "#" + window.hasher.getHash();
                } else {
                    sUrl = sUrl.split("#")[0] + sPlugins + "#" + window.hasher.getHash();
                }
            } else {
    ...
    var ApplicationContainer = Control.extend(sCOMPONENT, {
        metadata: {
            properties: {
                ...
                isPostCapability: {defaultValue: false, type: "boolean"}
            },
            events: {
