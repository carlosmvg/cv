    ...
    return Controller.extend("sap.ushell.components.tiles.cdm.applauncherdynamic.DynamicTile", {
        ...
        loadData: function (iServiceRefreshInterval) {
            return new Promise(function (resolve, reject) {
                var sSAPLogonLanguage,
                    sSapClient,
                    oRequestHeaders,
                    oURI,
                    oModel = this.getView().getModel(),
                    sUrl = oModel.getProperty("/configuration/serviceUrl"),
                    sContentProviderId = oModel.getProperty("/properties/contentProviderId");
                ...
                sap.ushell.Container.getServiceAsync("ClientSideTargetResolution")
                    ...
                    .then(function (oResults) {
                        ...
                        return oReferenceResolverService.resolveUserDefaultParameters(sUrl, oSystemContext);
                    })
                    .then(function (oResult) {
                        ...
                        if (iServiceRefreshInterval > 0) {
                            // log in English only
                            Log.info(
                                "Wait " + iServiceRefreshInterval + " seconds before calling " + oResult.url + " again", null,
                        ...
                        if (!this.oDataRequest) {
                            ...
                            if ((sSAPLogonLanguage) && (oResult.url.indexOf("sap-language=") === -1)) {
                                oResult.url = oResult.url + (oResult.url.indexOf("?") >= 0 ? "&" : "?") + "sap-language=" + sSAPLogonLanguage;
                            }

                            oModel.setProperty("/configuration/serviceUrl", oResult.url);
                            this.oDataRequest = OData.read(
                                {
                                    requestUri: oResult.url,
