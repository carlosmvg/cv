...
sap.ui.define([
            ...
            oServiceCalls: {
                ...
                "sendUrlAsEmail" : {
                    executeServiceCallFn: function (oServiceParams) {
                        sendEmail(
                            "",
                            sap.ushell.resources.i18n.getText("linkTo"),
                            document.URL,
                            "",
                            "",
                            "",
     ...
     * @private
     */
    function sendEmail(sTo, sSubject, sBody, sCc, sBcc, sIFrameURL, bSetAppStateToPublic) {
        if (bSetAppStateToPublic === true) {
        ...
        if (sIFrameURL && sIFrameURL.length > 0 && ((sSubject && sSubject.includes(sIFrameURL)) || (sBody && sBody.includes(sIFrameURL)))) {
            //Replace the URL
            if (sBody && sBody.includes(sIFrameURL)) {
                sBody = sBody.replace(sIFrameURL, document.URL);
            }
            if (sSubject && sSubject.includes(sIFrameURL)) {
                sSubject = sSubject.replace(sIFrameURL, document.URL);
            }
        }
