    ...
    return Controller.extend("sap.ushell.components.pages.controller.Pages", /** @lends sap.ushell.components.pages.controller.Pages.prototype */ {
        ...
        _getPageAndSpaceId: function () {
            return this._oURLParsingService.then(function (urlParsingService) {
                var sIntent = window.hasher.getHash();
                ...
                return this._parsePageAndSpaceId(aPageId, aSpaceId, sIntent);
            }.bind(this));
        },
         ...
         * @param {string} intent An object that contains the semantic object and action of the page which should be displayed
         * @returns {Promise<object>} Resolves to an object contains the pageId and spaceId
         * @private
         * @since 1.72.0
         */
        ...
        /**
         * Triggers the navigation to a specific Page after the pageId is returned and the VisualizationLoading
         * service loaded all the Visualization data and the Pages service could successfully load the requested Page.
        ...
        _visualizationFactory: function (id, context) {
            ...
            return new GenericTile({
                state: mLibrary.LoadState.Failed
            });
        },
        ...
        moveVisualization: function (oEvent, oSource, oParameters) {
            ...
            // Do not DnD from normal section to a default section
            if (oTargetSection.getDefault() && !oSourceSection.getDefault()) {
                return Promise.resolve();
            }
            // invalidate dragged control to unselect it by rerendering it
            oParameters.draggedControl.invalidate();

            // don't set the promise if source is default section and only has one visualization
            // this section might be deleted and the promise not resolved
            if (!(oSourceSection.getDefault() && oSourceSection.getVisualizations().length === 1)) {
                this._setPromiseInSection(oSourceSection, oTargetSection);
            }

            if (oTargetSection._focusItem) {
                oTargetSection._focusItem(iTargetVisualizationIndex); // Make sure that the visualization remains focused after move
            }

            // Adapt target index for the service.
            ...
            return oComponent.getPagesService()
                .then(function (oPagesService) {
                    return oPagesService.moveVisualization(
                        iPageIndex,
                        iSourceSectionIndex,
                        iSourceVisualizationIndex,
                        iTargetSectionIndex,
                        iTargetVisualizationIndex
                    );
                })
                .then(function () {
                    var sVizMovedMessage = resources.i18n.getText("PageRuntime.Message.TileMoved");
                    oComponent.getInvisibleMessageInstance().announce(sVizMovedMessage, coreLibrary.InvisibleMessageMode.Polite);
                });
        },
