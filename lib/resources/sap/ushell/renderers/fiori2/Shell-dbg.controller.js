...
sap.ui.define([
    ...
    sap.ui.controller("sap.ushell.renderers.fiori2.Shell", {
            ...
            window.onbeforeunload = function () {
                if (sap.ushell.Container && sap.ushell.Container.getDirtyFlag()
                    ...
                    && !(Device.browser.name === Device.browser.BROWSER.INTERNET_EXPLORER
                         && AppLifeCycle.getCurrentApplication().container.getApplicationType() === sap.ushell.ApplicationType.NWBC.type)
                ) {
        ...
        _doHashChange: function (oShellController, sShellHash, sAppPart, sOldShellHash, sOldAppPart, oParseError) {
            ...
            return oShellController._resolveHashFragment(sFixedShellHash)
                .then(function (oResolvedHashFragment, oParsedShellHash) {
                    ...
                    if (oShellController._handleEarlyNavigation(sFixedShellHash, sAppPart, oResolvedHashFragment)) {
                        return;
                    }
        ...
        _handleEarlyNavigation: function (sFixedShellHash, sAppPart, oResolvedHashFragment) {
            var bNwbcHandling = oResolvedHashFragment && (
                    utils.isNativeWebGuiNavigation(oResolvedHashFragment)
                    || oResolvedHashFragment.nativeNWBCNavigation),
                oDashboard;

            if (bNwbcHandling && this._openAppViaNWBC(oResolvedHashFragment)) {
            ...
            if (Config.last("/core/shell/model/migrationConfig")) {
                ...
                this.getModel().setProperty("/migrationConfig", false);
         ...
         * @return {boolean}
         *  Whether NWBC managed to open the application.
         *
         * @private
         */
        _openAppViaNWBC: function (oResolvedHashFragment) {
            try {
                ...
                if (!oEpcm.doNavigate(sUrlWithSapUserAndShellParam, iMode)) {
                    return false;
                }
