...
sinaDefine(['../../core/core', './typeConverter'], function (core, typeConverter) {
    ...
    return core.defineClass({
        ...
        fillMetadataBuffer: function (dataSource, attributes, log) {
            ...
            for (var attributeMetadata in attributes.attributeMap) {
                try {
                    this.fillPublicMetadataBuffer(dataSource, attributes.attributeMap[attributeMetadata], cdsAnnotations, log);
                } catch (e) {
        ...
        fillPublicMetadataBuffer: function (dataSource, attributeMetadata, cdsAnnotations, log) {
            ...
            var typeAndFormat = this._parseAttributeTypeAndFormat(attributeMetadata, dataSource, log);
        ...
        _parseAttributeTypeAndFormat: function (attributeMetadata, dataSource, log) {
            ...
            default:
                if (attributeMetadata.type && attributeMetadata.type.startsWith('Collection')) {
                    log.addMessage(log.WARNING, 'Unsupported data type ' + attributeMetadata.type + ' of attribute ' + attributeMetadata.labelRaw + ' in ' + dataSource.label);
                    return {
                        type: this.sina.AttributeType.String
                    };
                }
                log.addMessage(log.ERROR, 'Unknown data type ' + attributeMetadata.type + ' of attribute ' + attributeMetadata.labelRaw + ' in ' + dataSource.label);
                return null;
            }
        },
        ...
        parseDynamicMetadata: function (searchResult, log) {
            ...
            for (var attributeId in metadata) {
                ...
                this.parseDynamicAttributeMetadata(dataSource, attributeId, dynamicAttributeMetadata, log);
            }

        },

        parseDynamicAttributeMetadata: function (dataSource, attributeId, dynamicAttributeMetadata, log) {
