...
sap.ui.define([
    ...
    VizInstance.prototype.setState = function (state) {
        ...
        switch (state) {
            case VisualizationLoadState.InsufficientRoles:
            case VisualizationLoadState.OutOfRoleContext:
                sInnerControlLoadState = LoadState.Failed;
                sFailedText = resources.i18n.getText("VisualizationInstance.OutOfRoleContext");
                break;
        ...
        if (oInnerControl && oInnerControl.setState) {
            ...
            if (oInnerControl.setFailedText && sFailedText) {
                oInnerControl.setFailedText(sFailedText);
            }
        } else {
            this._getInnerControlPromise().then(function (innerControl) {
                ...
                if (innerControl.setFailedText && sFailedText) {
                    innerControl.setFailedText(sFailedText);
                }
            });
        }
    };
    ...
    VizInstance.prototype.getFocusDomRef = function () {
        if (!this.getEditable()) {
            var aPossibleControls = this.getInnerControl().findAggregatedObjects(true, function (oControl) {
        ...
        return this.getInnerControl().getFocusDomRef();
    };
