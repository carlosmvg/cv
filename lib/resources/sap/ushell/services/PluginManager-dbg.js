...
sap.ui.define([
...
], function (fnGetExtensions, Log, EventHub, jQuery, UriParameters, utils, PostMessageAPIInterface) {
    "use strict";
    ...
    function PluginManager (oContainerInterface, sParameter, oServiceProperties) {
        var that = this,
            aComponentsWithAgent = [],
            bisComponentsLoaded = false,
            oLifeCycleManager;
            ...
            function loadComponent () { // not functional
                sap.ushell.Container.getService("Ui5ComponentLoader").createComponent(oApplicationProperties)
                    .done(function (oLoadedComponent) {
                        if (oPlugin.config && oPlugin.config["sap-component-agents"]) {
                            aComponentsWithAgent.push({
                                componentName: oPlugin.component,
                                url: oPlugin.url,
                                agents: oPlugin.config["sap-component-agents"],
                                pluginComp: oLoadedComponent
                            });
                        }
                        if (oPluginDeferred) {
        ...
        this.registerPlugins = function (oPlugins) {
            ...
            if (this._oConfig.isBlueBox === true) {
                ...
                EventHub.emit("pluginConfiguration", oPlugins);
            }
            ...
            Object.keys(oPlugins).sort().forEach(function (sPluginName) {
                ...
                if (that._oConfig.isBlueBox === true) {
                    if (oCurrentPlugin.config && oCurrentPlugin.config["sap-plugin-agent"] === true) {
                        ...
                        if (!that.isPluginAgentSupported(sSapAgentId)) {
                            if (sSapAgentsIds) {
            ...
            try {
                ...
                jQuery.sap.log.error("failed to build plugin agents names list",
        ...
        /**
         * Registers A LifeCycle manager for the plugins enables managed plugin agents, once the plugins loaded call the callback "inLifeCycleManager" wii be called with the list of created plugins.
         * This will enable to the lice cycle to call interfaces on the different plugins.
         *
         * @param {function} inLifeCycleManager a function that will manage the agentStart / agentLoading / agentExit.

         * @since 1.78
         * @private
         */
        this.registerAgentLifeCycleManager = function (inLifeCycleManager) {
            oLifeCycleManager = inLifeCycleManager;

            if (bisComponentsLoaded) {
                oLifeCycleManager(aComponentsWithAgent);
            }
        };
                    ...
                    if (aPluginPromises.length > 0) {
                        jQuery.when.apply(undefined, aPluginPromises)
                            .done(function () {
                                ...
                                if (oLifeCycleManager) {
                                    oLifeCycleManager(aComponentsWithAgent);
                                }

                                bisComponentsLoaded = true;
