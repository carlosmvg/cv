...
sap.ui.define([
     ...
     *
     *       Meaning that the function accepts the first argument as argNameX,
     *       and optionally argNameY as a second argument.
     *
     *   it will be possible to access the arguments through their names as
     *   oArgs.argNameX, or oArgs.argNameY (may be be undefined)
     *
     * - vValues: an array of values
     *
     *   Because it's possible to specify more arguments, vValues will be
     *   always an array the first time the expression is parsed. Even when a
     *   single value is named into the expresison.
     *
     *   A function that transforms its input should:
     *
     *   - Attempt to reduce arrays of complex values to a single simple value,
     *     and the single value (not the array of a single value should be
     *     returned). This is because in the end the target place of a single
     *     value is the url where all values become a string.
     *
     *   - Return a complex value if the function does not reduce multiple
     *     values to a simple single value (string, number, null). Better if the
     *     returned value keeps the same type as the input value. For example,
     *     if multiple values were given, vValues is an array, an array shall
     *     be returned.
     *
     *   - Treat an array of a single value as a single value, and return the
     *     single value.
     *
     *     If this is not done, the following can happen when the value is
     *     transferred across multiple expressions:
     *
     ...
     *
     *     When the function unpacks the array with a single item:
     *
     *     nameA: {fn() value}   // vValues: [value]  -> returns value
     *     nameB: {fn() nameA}   // vValues: [value] -> returns value
     *     nameC: {fn() nameB}   // vValues: [value] ... and so on
     ...
     *     which is nicer.
     *
     * - The number of items in vValues can be controlled by minValues and
     *   maxValues, which will cause the engine to throw during parsing.
     */
    ...
    var O_FUNCTION = {
        ...
        replace: {
            ...
            fn: function (oArgs, vValues) {
                ...
                if (oType.isPlainObject(vValues)) {
