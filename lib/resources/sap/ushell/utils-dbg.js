...
sap.ui.define([
    ...
    /**
     * Detect whether NWBC can be notifed that the Container and its services are ready to be used
     * and NWBC also supports the using of FLP interface for calling some FLP apis exposed to it
     * via interface
     *
     * NWBC exposes a feature bit vector via the getNwbcFeatureBits method of the private epcm object.
     * This is expected to be a string in hex format representing 4 bits, where the most significant bit represents the flp ready notification capability.
    ...
    /**
     * Checks whether the given intent is the configured root intent (apart
     * from its parameters).
     *
     * #Shell-home should never be hardcoded as it can be configured.
     *
     * @param {string} sIntent
     *    The intent to check, e.g., <code>#Employee-display</code>
     *
     *    NOTE: The initial hash fragment is ignored during the check. For
     *    example if <code>#Employee-display</code> matches the root intent,
     *    also Employee-display does.
     *
     *
     * @returns {boolean}
     *    Whether the given intent is the root intent.
     *
     * @throws {Error}
     *    When a wrong input parameter is given.
     *
     * @private
     */
